...
| INV-X14 | **Only well-formed requests are proposed, and every proposed command applies.** A request outside the contract is `INVALID_REQUEST` before anything is proposed; one that validates becomes a log command every replica decodes and applies. | `TestRequestValidationRejectsEveryOutOfContractField`, `TestValidatedRequestsAlwaysApply`, `TestMalformedIdentifiersAreProtocolErrors`, `TestDurationsThatOverflowAreProtocolErrors`, the wire and command fuzz targets (decoding is total and canonical); mutants 87, 90 | VERIFIED |
