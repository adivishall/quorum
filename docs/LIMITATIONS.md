...
finite histories, honest recording of retries. There is still no client/HTTP API, no request
forwarding or dedup, no multi-group routing, no LSM engine behind Raft, no snapshots, and no
dynamic membership.
...
| **A client that hides a retry of an unknown write is outside the guarantee.** Without request ids a retried write can be applied twice; recorded as two operations the history is linearizable, collapsed into one it is not (`TestRealIncompleteWriteThenRetry`). The Phase 12 test client never retries an unknown write. | Phase 13 (request ids, dedup: INV-X2) |
...
| **The client protocol is a test boundary, not an API.** `dkvd -client-listen` speaks a minimal framed-TCP PUT/GET/DELETE protocol with no request ids, no forwarding (a non-leader answers "not leader" with a hint), no authentication and no deduplication. It exists so real processes can be driven by history-recording clients. | Phase 13 (redirection/forwarding, ids), Phase 15 (HTTP API) |
