...
**Status: Phase 12.** A single-node key-value store with a durable write-ahead log and an
...
`docs/DEDUP.md`, `docs/API.md`): retries of identified writes are now inside the guarantee. There is
still no HTTP API, no multi-group routing, no LSM engine behind Raft, no snapshots, and no dynamic
membership.
...
| **Linearizability is verified on recorded, finite histories — not proven for every execution.** Every history recorded by the real-process, in-process and simulator tiers is linearizable and the write-completion and ReadIndex mechanisms are argued with named assumptions (`docs/LINEARIZABILITY.md` §3, §5.2), but runs are minutes long, clusters have 3 or 5 nodes, one Raft group, and ≤ 8 clients. Longer, larger and multi-group histories are untested. Seeded random schedules are a net, not a proof: they found the no-quorum mutant only after a two-sided-split profile existed, and then in ~9% of runs (the scripted attacks are the deterministic killers). | more tiers in later phases (multi-group with routing; soak runs) |
| **An anonymous write retried after an unknown outcome can be applied twice.** Identified writes (a session's ClientID and RequestID) are deduplicated (INV-X2); anonymous ones (ClientID 0) keep Phase 12's semantics — recorded as two operations the history is linearizable, collapsed into one it is not (`TestRealIncompleteWriteThenRetry`). | deliberate — the anonymous mode exists to keep the Phase 12 contract meaningful |
...
| **The session table is rebuilt by replaying the whole log**, like the rest of the in-memory state machine, and costs up to 13.4 MiB at the default limits. Phase 14's snapshots must include it, or a retry after a restore would execute again. | Phase 14 (snapshots) |
...
| **The client protocol is framed binary TCP, not an HTTP API.** `dkvd -client-listen` speaks wire protocol v2 (`docs/API.md`): PUT/GET/DELETE/REGISTER with request identity, one-hop forwarding (or redirect-only), eleven statuses — with no authentication, no TLS and no version negotiation. | Phase 15 (HTTP API) |
...
| **The replicated state machine is in memory and rebuilt by replay on every restart** (`kv.Store`), so restart cost grows with the Raft log, which is never truncated. | engine wiring (the LSM engine as the state machine), Phase 14 (snapshots) |
...
| **A node hosts one Raft group and no state machine of consequence.** `internal/raftnode` drives a single group with a minimal (often nil / recording) state machine; it is not wired to the LSM engine, hosts no shards, and the multi-Raft (one group per shard) node is a later phase. **`appliedIndex` is volatile:** every restart re-applies the whole recovered committed prefix, so state-machine application is **at-least-once across restarts** and exactly-once only within an incarnation (Phase 11, INV-CR4). A state machine that needs idempotence must record its own applied index (the engine will, `docs/DESIGN.md` §10 step 7); nothing in the driver deduplicates. (Client-level deduplication, Phase 13, is a different thing: the replay rebuilds the session table, and a retried request is recognized as a duplicate of an earlier *entry* — `docs/DEDUP.md` §4.) | per-shard node, engine wiring |
| No HTTP API, no stale-mode reads, no dashboard. (Phases 12–13 serve PUT/GET/DELETE with linearizable reads, request identity, deduplication and forwarding on the framed protocol — see the rows above for its bounds.) | Phases 15–17 |
